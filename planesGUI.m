...
for k2 = 1:numel(uistate.plane_list)
    if(isfield(uistate.plane_list,'region') && ~isempty(uistate.plane_list(k2).region))
        if(~iscell(uistate.plane_list(k2).region))
          uistate.plane_list(k2).region = {uistate.plane_list(k2).region};
        end
        for i = 1:numel(uistate.plane_list(k2).region)
          u = uistate.plane_list(k2).region{i};
          color = mpdc(k2,:);
          hold on;
          plot(gca,[u(1,:) u(1,1)], [u(2,:) u(2,1)], ...
               'Color',color,'LineWidth',3);
          mu = [sum(u(1,:))/size(u,2); sum(u(2,:))/size(u,2)];
          text(mu(1,:)',mu(2,:)', ...
               num2str(k2), ...
               'Color',color);
          hold off;
...
point = point(1,1:2);
if(~isempty(uistate.id))
  uistate.plane_list(uistate.id).region{ ...
    numel(uistate.plane_list(uistate.id).region) + 1} = ...
       getPosition(uistate.h)';
  uistate.plane_list(uistate.id).region
end
uistate.id = get_plane_id(uistate.plane_list,point);
if(~isfield(uistate.plane_list(uistate.id),'region'))
  uistate.plane_list(uistate.id).region = [];
end
uistate.id
uistate.h = impoly(gca); 
guidata(gcf,uistate);
